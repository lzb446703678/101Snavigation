<template>
  ...
  <Transition name="fade" mode="out-in">
    <div v-if="shortcutlearnData[0]" class="shortcut">
      <n-scrollbar class="scrollbar">
        ...
        >
          <n-grid-item
            v-for="item in shortcutlearnData"
            :key="item"
            class="shortcut-item"
...
const { shortcutlearnData } = storeToRefs(site);
...
const addShortcutModalOpen = () => {
  // 生成 ID
  const shortcutMaxID = shortcutlearnData.value.reduce((max, item) => {
...
const addOrEditShortcuts = () => {
  addShortcutRef.value?.validate((errors) => {
    ...
    if (!addShortcutModalType.value) {
      // 是否重复
      const isDuplicate = shortcutlearnData.value?.some(
        (item) =>
      ...
      // 编辑捷径
      const index = shortcutlearnData.value.findIndex((item) => item.id === addShortcutValue.value.id);
      if (index === -1) {
        $message.error("捷径中不存在该项，请重试");
        return false;
      }
      shortcutlearnData.value[index].name = addShortcutValue.value.name;
      shortcutlearnData.value[index].url = addShortcutValue.value.url;
...
const delShortcuts = () => {
  ...
  if (typeof deleteId === "number") {
    const indexToRemove = shortcutlearnData.value.findIndex((item) => item.id === deleteId);
    if (indexToRemove !== -1) {
      shortcutlearnData.value.splice(indexToRemove, 1);
      // 将后续元素的 id 前移一位
      for (let i = indexToRemove; i < shortcutlearnData.value.length; i++) {
        shortcutlearnData.value[i].id = i;
      }
