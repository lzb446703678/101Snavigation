<template>
  ...
  <Transition name="fade" mode="out-in">
    <div v-if="shortcutvideoData[0]" class="shortcut">
      <n-scrollbar class="scrollbar">
        ...
        >
          <n-grid-item
            v-for="item in shortcutvideoData"
            :key="item"
            class="shortcut-item"
...
const { shortcutvideoData } = storeToRefs(site);
...
const addShortcutModalOpen = () => {
  // 生成 ID
  const shortcutMaxID = shortcutvideoData.value.reduce((max, item) => {
...
const addOrEditShortcuts = () => {
  addShortcutRef.value?.validate((errors) => {
    ...
    if (!addShortcutModalType.value) {
      // 是否重复
      const isDuplicate = shortcutvideoData.value?.some(
        (item) =>
      ...
      // 编辑捷径
      const index = shortcutvideoData.value.findIndex((item) => item.id === addShortcutValue.value.id);
      if (index === -1) {
        $message.error("捷径中不存在该项，请重试");
        return false;
      }
      shortcutvideoData.value[index].name = addShortcutValue.value.name;
      shortcutvideoData.value[index].url = addShortcutValue.value.url;
...
const delShortcuts = () => {
  ...
  if (typeof deleteId === "number") {
    const indexToRemove = shortcutvideoData.value.findIndex((item) => item.id === deleteId);
    if (indexToRemove !== -1) {
      shortcutvideoData.value.splice(indexToRemove, 1);
      // 将后续元素的 id 前移一位
      for (let i = indexToRemove; i < shortcutvideoData.value.length; i++) {
        shortcutvideoData.value[i].id = i;
      }
